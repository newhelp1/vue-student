            ...
            <div>
                <el-tabs :tab-position="tabPosition" v-model="activeName" style="height: 200px;">
                    <el-tab-pane label="密码登录" name="first">
                        <el-form @submit.native.prevent ref="adminFormRef" :model="admin" :rules="adminFormRules"
                            ...
                            <el-form-item prop="name">
                                <el-input size="medium" v-model="admin.name"
                                          prefix-icon="el-icon-user-solid"></el-input>
                            </el-form-item>
                            <!--                密码-->
                            <el-form-item prop="password">
                                <el-input size="medium" v-model="admin.password" prefix-icon="el-icon-lock"
                            ...
                            <el-form-item>
                                ...
                                <el-link href="http://localhost:8080/teacherLogin" type="success">教师登录</el-link>
                    ...
                    <el-tab-pane label="邮箱登录" name="second">
                        <div>
                            <el-form @submit.native.prevent ref="adminFormRefEmail" :model="admin"
                                     :rules="adminFormRulesEamil"
                                     label-width="0px"
                                     class="login_form">
                                <!--                用户名-->
                                <el-form-item prop="email">
                                    <el-input size="medium" v-model="admin.email" prefix-icon="el-icon-user"></el-input>
                                </el-form-item>
                                <!--                密码-->
                                <el-form-item prop="code">
                                    <el-input size="medium" v-model="admin.code" prefix-icon="el-icon-message"
    ...
    export default {
        ...
        methods: {
            ...
            sendEmailCode(type) {
                ...
                //发送邮箱验证码
                this.request.get("http://localhost:9090/admin/email/" + email + "/" + type).then(res => {
                    if (res.code === '200') {
            ...
            passwordBack() {
                this.request.put("http://localhost:9090/admin/reset", this.pass).then(res => {
                    if (res.code === '200') {
            ...
            loginEmail() {
                this.$refs['adminFormRefEmail'].validate((valid) => {
                    if (valid) {  //表单校验合法
                        this.request.post("http://localhost:9090/admin/loginEmail", this.admin).then(res => {
                            if (res.code === '200') {
                                localStorage.setItem("admin", JSON.stringify(res.data))//存储用户信息到浏览器
                                this.$message.success('登陆成功');
                                this.$router.push('/home')
