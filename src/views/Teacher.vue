<template>
    <div>
            ...
            >
                ...
                <el-form class="add" size="small" label-width="130px" :model="add" :rules="addRules"
                    ...
                    <el-form-item label="教师编号" prop="no">
                        <el-input v-model.trim="add.no" placeholder="请输入教师编号"></el-input>
                    </el-form-item>
                    <el-form-item label="教师账号" prop="name">
                        <el-input v-model.trim="add.name" placeholder="请输入教师账号"></el-input>
                    </el-form-item>
                    <el-form-item label="教师姓名" prop="nickname">
                        <el-input v-model.trim="add.nickname" placeholder="请输入教师姓名"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="教师邮箱" prop="email">
                        <el-input v-model.trim="add.email" placeholder="请输入教师邮箱"></el-input>
                    </el-form-item>
        ...
        <div class="addAndDelete" >
            <el-button type="primary" icon="el-icon-circle-plus-outline" size="small" @click="addBtn">新增教师
            </el-button>
            <el-popconfirm
        ...
        <el-table @selection-change="handleSelectionChange"
            ...
            <el-table-column align="center" prop="no" label="教师编号"></el-table-column>
            <el-table-column align="center" prop="nickname" label="教师姓名">
            </el-table-column>
...
<script scoped>
    ...
    export default {
        ...
        methods: {
            ...
            //修改状态
            changeStatus(row) {
                let teacher =localStorage.getItem("teacher") ? JSON.parse(localStorage.getItem("teacher")) : {}
                if (teacher.id===row.id && !row.status){
                    row.status=true
                    this.$message.warning('禁止修改本人状态');
                    return
                }
                this.request.post("teacher", row).then(res => {
                    // console.log(row)
                    if (res.code === '200') {
                        //可以发起添加用户网络请求
            ...
            saveDialogBtn() {
                this.$refs['addFormRef'].validate((valid) => {
                    if (valid) {
                        this.request.post("teacher", this.add).then(res => {
                            if (res.code === '200') {
                                //可以发起添加用户网络请求
                                this.$message.success('操作成功');
                                //隐藏添加对话框
