            ...
            <div>
                <el-tabs :tab-position="tabPosition" v-model="activeName" style="height: 200px;">
                    <el-tab-pane label="教师登录" name="first">
                        <el-form @submit.native.prevent ref="teacherFormRef" :model="teacher" :rules="teacherFormRules"
                            ...
                            <el-form-item prop="name">
                                <el-input size="medium" v-model="teacher.name"
                                          prefix-icon="el-icon-user-solid"></el-input>
                            </el-form-item>
                            <!--                密码-->
                            <el-form-item prop="password">
                                <el-input size="medium" v-model="teacher.password" prefix-icon="el-icon-lock"
                                          show-password></el-input>
                            </el-form-item>

                            <!--                按钮区-->
                            <el-form-item class="btns">
                                <el-button size="small" plain type="success" @click="login" native-type="submit">登录
                                </el-button>
                            ...
                            <el-form-item>
                                ...
                                <el-link href="http://localhost:8080/studentLogin" style="margin-left: 8px"
                                         type="primary">学生登录
                                </el-link>
                            </el-form-item>
                        </el-form>
                    ...
                    <el-tab-pane label="邮箱登录" name="second">
                        <div>
                            <el-form @submit.native.prevent ref="teacherFormRefEmail" :model="teacher"
                                     :rules="teacherFormRulesEamil"
                                     label-width="0px"
                                     class="login_form">
                                <!--                用户名-->
                                <el-form-item prop="email">
                                    <el-input size="medium" v-model="teacher.email"
                                              prefix-icon="el-icon-user"></el-input>
                                </el-form-item>
                                <!--                密码-->
                                <el-form-item prop="code">
                                    <el-input size="medium" v-model="teacher.code" prefix-icon="el-icon-message"
                                              style="width: 360px"></el-input>
                                    <el-button size="medium" plain type="primary" style="margin-left: 5px;"
                                ...
                                <!--                按钮区-->
                                <el-form-item class="btns">
                                    <el-button size="small" plain type="success" @click="loginEmail"
                                               native-type="submit">登录
                                    </el-button>
                                    <el-button size="small" plain type="danger" @click="resetLoginFromEamil">重置
                                    </el-button>
                                </el-form-item>
                                <el-form-item>
                                    <el-button class="link"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //重置邮箱登录表单
            resetLoginFromEamil() {
                this.$refs.teacherFormRefEmail.resetFields();
            },
            ...
            sendEmailCode(type) {
                ...
                //发送邮箱验证码
                this.request.get("http://localhost:9090/teacher/email/" + email + "/" + type).then(res => {
                    if (res.code === '200') {
            ...
            passwordBack() {
                this.request.put("http://localhost:9090/teacher/reset", this.pass).then(res => {
                    if (res.code === '200') {
            ...
            login() {
                this.$refs['teacherFormRef'].validate((valid) => {
                    if (valid) {  //表单校验合法
                        this.request.post("http://localhost:9090/teacher/login", this.teacher).then(res => {
                            if (res.code === '200') {
                                localStorage.setItem("teacher", JSON.stringify(res.data))//存储用户信息到浏览器
            ...
            loginEmail() {
                this.$refs['teacherFormRefEmail'].validate((valid) => {
                    if (valid) {  //表单校验合法
                        this.request.post("http://localhost:9090/teacher/loginEmail", this.teacher).then(res => {
                            if (res.code === '200') {
                                localStorage.setItem("teacher", JSON.stringify(res.data))//存储用户信息到浏览器
                                this.$message.success('登陆成功');
                                this.$router.push('/home')
